...
import SHAMonitor from "@/components/SHAMonitor";

export const dynamic = "force-dynamic";

async function getData() {
  const [tradesRaw, equityRaw, activeRaw, marketRaw] = await Promise.all([
    fetchOkxFile("results/live_investment_sha/trades.csv").catch(() => ""),
    fetchOkxFile("results/live_investment_sha/equity_sha.csv").catch(() => ""),
    fetchOkxFile("results/live_investment_sha/active_state_sha.json").catch(() => "{}"),
    fetchOkxFile("results/live_investment_sha/market_state_sha.json").catch(() => "{}"),
  ]);
...
export default async function SHADashboard() {
    ...
    <main className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-400 text-sm">
          SHA Heikin Ashi Reversal System — Bybit Demo Account
        </p>
      </div>
      ...
      <StatsHeader
        ...
        activeApiPath="/api/sha/active"
        marketStateApiPath="/api/sha/market-state"
      />

      {/* Open trades */}
      <LiveStatus activeApiPath="/api/sha/active" />

      {/* Per-symbol SHA state monitor */}
      <SHAMonitor />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <EventsFeed apiPath="/api/sha/events" />
      </div>
      ...
      <footer className="text-center text-xs text-slate-600 py-4 border-t border-surface-border">
        Bybit demo account · SHA Heikin Ashi Reversal system · Not financial advice
      </footer>
    </main>
  );
}
