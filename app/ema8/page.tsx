...
async function getData() {
  const [tradesRaw, equityRaw, activeRaw, marketRaw] = await Promise.all([
    fetchOkxFile("results/live_investment_ema8/trades.csv").catch(() => ""),
    fetchOkxFile("results/live_investment_ema8/equity_ema8.csv").catch(() => ""),
    fetchOkxFile("results/live_investment_ema8/active_state_ema8.json").catch(() => "{}"),
    fetchOkxFile("results/live_investment_ema8/market_state_ema8.json").catch(() => "{}"),
  ]);
...
export default async function EMA8Dashboard() {
    ...
    <main className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <h1 className="text-xl sm:text-2xl font-bold text-white">EMA Reclaim + Retest — EMA8</h1>
        ...
        <p className="text-slate-400 text-sm">
          Price was above/below EMA8, violates it, shows resistance and reclaims (closes back on
          trend side), then a limit order retests EMA8 for entry · sl=0.5×ATR · tp=2R
        </p>
        <p className="text-xs text-slate-500">
          Risk sizing: <span className="text-slate-300 font-medium">0.167% of equity per trade</span> —
          the smaller of (a) target max 15% historical drawdown at backtest DD=90.0R, and
          (b) capping a worst-case day (all ~16.0 trades/day losing) at 3% of equity. Backtest:
          n=30,065 · EV=+0.361R · WR=57.7% · PF=1.62 · 6/6 years positive (2021–2026, all 6 symbols).
        </p>
      </div>
      ...
      <StatsHeader
        ...
        activeApiPath="/api/ema8/active"
        marketStateApiPath="/api/ema8/market-state"
      />

      <LiveStatus activeApiPath="/api/ema8/active" />

      <EMAMonitor apiPath="/api/ema8/monitor" emaPeriod={8} />

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        <EquityChart data={equity} trades={trades} currentEquity={currentEquity} startingEquity={startingEquity} />
        <EventsFeed apiPath="/api/ema8/events" />
      </div>
      ...
      <TradeHistory trades={trades} apiPath="/api/ema8/trades" />

      <footer className="text-center text-xs text-slate-600 py-4 border-t border-surface-border">
        Bybit demo account · EMA Reclaim + Retest (EMA8) · Not financial advice
      </footer>
    </main>
  );
}
