...
import VWMonitor from "@/components/VWMonitor";

export const dynamic = "force-dynamic";

async function getData() {
  const [tradesRaw, equityRaw, activeRaw, marketRaw] = await Promise.all([
    fetchOkxFile("results/live_investment_vw/trades.csv").catch(() => ""),
    fetchOkxFile("results/live_investment_vw/equity_vw.csv").catch(() => ""),
    fetchOkxFile("results/live_investment_vw/active_state_vw.json").catch(() => "{}"),
    fetchOkxFile("results/live_investment_vw/market_state_vw.json").catch(() => "{}"),
  ]);
...
export default async function VWDashboard() {
    ...
    <main className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <h1 className="text-xl sm:text-2xl font-bold text-white">VWAP Reversion</h1>
      ...
      <StatsHeader
        ...
        activeApiPath="/api/vw/active"
        marketStateApiPath="/api/vw/market-state"
      />

      <LiveStatus activeApiPath="/api/vw/active" />

      <VWMonitor />

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        <EquityChart data={equity} trades={trades} currentEquity={currentEquity} startingEquity={startingEquity} />
        <EventsFeed apiPath="/api/vw/events" />
      </div>
