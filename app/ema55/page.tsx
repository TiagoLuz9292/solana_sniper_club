...
async function getData() {
  const [tradesRaw, equityRaw, activeRaw, marketRaw] = await Promise.all([
    fetchOkxFile("results/live_investment_ema55/trades.csv").catch(() => ""),
    fetchOkxFile("results/live_investment_ema55/equity_ema55.csv").catch(() => ""),
    fetchOkxFile("results/live_investment_ema55/active_state_ema55.json").catch(() => "{}"),
    fetchOkxFile("results/live_investment_ema55/market_state_ema55.json").catch(() => "{}"),
  ]);
...
export default async function EMA55Dashboard() {
    ...
    <main className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <h1 className="text-xl sm:text-2xl font-bold text-white">EMA Reclaim + Retest — EMA55</h1>
        ...
        <p className="text-slate-400 text-sm">
          Price was above/below EMA55, violates it, shows resistance and reclaims (closes back on
          trend side), then a limit order retests EMA55 for entry · sl=0.5×ATR · tp=2R
        </p>
        <p className="text-xs text-slate-500">
          Risk sizing: <span className="text-slate-300 font-medium">0.259% of equity per trade</span> —
          the smaller of (a) target max 15% historical drawdown at backtest DD=57.9R, and
          (b) capping a worst-case day (all ~5.7 trades/day losing) at 3% of equity. Backtest:
          n=10,636 · EV=+0.374R · WR=58.7% · PF=1.65 · 6/6 years positive (2021–2026, all 6 symbols).
        </p>
      </div>
      ...
      <StatsHeader
        ...
        activeApiPath="/api/ema55/active"
        marketStateApiPath="/api/ema55/market-state"
      />

      <LiveStatus activeApiPath="/api/ema55/active" />

      <EMAMonitor apiPath="/api/ema55/monitor" emaPeriod={55} />

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        <EquityChart data={equity} trades={trades} currentEquity={currentEquity} startingEquity={startingEquity} />
        <EventsFeed apiPath="/api/ema55/events" />
      </div>
      ...
      <TradeHistory trades={trades} apiPath="/api/ema55/trades" />

      <footer className="text-center text-xs text-slate-600 py-4 border-t border-surface-border">
        Bybit demo account · EMA Reclaim + Retest (EMA55) · Not financial advice
      </footer>
    </main>
  );
}
