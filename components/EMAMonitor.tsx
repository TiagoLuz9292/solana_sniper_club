...
function StateCard({ symbol, data, emaPeriod }: { symbol: string; data: SymbolState | undefined; emaPeriod: number }) {
  ...
  return (
    <div className={`bg-surface rounded-lg border ${borderColor} p-4 flex flex-col gap-2 min-w-0`}>
      ...
      {state === "IDLE" && (
        <p className="text-xs text-slate-500">Scanning for EMA{emaPeriod} reclaim signals</p>
      )}
      ...
      {state === "PENDING" && data && "level" in data && (
        <div className="space-y-1 text-xs">
          ...
          <p className="text-slate-400">
            EMA{emaPeriod} limit @ <span className="text-white font-mono">{data.level.toPrecision(6)}</span>
          </p>
          <p className="text-slate-400">
            Bar {data.bars_since_order} / {data.max_fill} — waiting for retest
          </p>
        </div>
      )}
...
export default function EMAMonitor({ apiPath, emaPeriod }: { apiPath: string; emaPeriod: number }) {
  ...
  if (!data || Object.keys(data).length === 0) {
    return (
      <div className="bg-surface rounded-xl border border-surface-border p-4">
        <h2 className="text-sm font-semibold text-slate-400 mb-3">EMA{emaPeriod} Symbol States</h2>
  ...
  return (
    <div className="bg-surface rounded-xl border border-surface-border p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-white">EMA{emaPeriod} Symbol States</h2>
        {lastUpdate && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
        {SYMBOLS.map((sym) => (
          <StateCard key={sym} symbol={sym} data={data[sym] as SymbolState | undefined} emaPeriod={emaPeriod} />
        ))}
      </div>
    </div>
  );
