...
function StateCard({ symbol, data }: { symbol: string; data: SymbolState | undefined }) {
  ...
  return (
    <div className={`bg-surface rounded-lg border ${borderColor} p-4 flex flex-col gap-2 min-w-0`}>
      ...
      {state === "IDLE" && (
        <p className="text-xs text-slate-500">Scanning for HA reversals</p>
      )}
...
export default function SHAMonitor() {
  const [data, setData] = useState<MonitorData | null>(null);
  const [lastUpdate, setLastUpdate] = useState<string>("");

  async function poll() {
    try {
      const res = await fetch("/api/sha/monitor", { cache: "no-store" });
      if (res.ok) {
  ...
  if (!data || Object.keys(data).length === 0) {
    return (
      <div className="bg-surface rounded-xl border border-surface-border p-4">
        <h2 className="text-sm font-semibold text-slate-400 mb-3">SHA Symbol States</h2>
  ...
  return (
    <div className="bg-surface rounded-xl border border-surface-border p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-white">SHA Symbol States</h2>
        {lastUpdate && (
